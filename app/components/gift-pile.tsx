...
type GiftPileSource = "received" | "sent";
...
const GiftPile: React.FC<Props> = ({ gifts, source }: Props) => {
  function handleGiftClicked(giftId: string): void {
    if (source === "received") {
      events.track(hGiftsOpenReceivedGift(giftId));
    }
    if (source === "sent") {
      events.track(hGiftsOpenSentGift(giftId));
    }
  }
