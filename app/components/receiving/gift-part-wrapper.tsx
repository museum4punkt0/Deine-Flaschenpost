...
class GiftPartWrapper extends React.PureComponent<Props, State> {
  ...
  public getPanelCount(): number {
    const giftPartIndex = getGiftPartIndexInGift(
      this.props.gift,
      this.props.giftPart
    );
  ...
  public render() {
    ...
    return (
      <StyledGiftPart
        imageSrc={this.props.giftPart.photo}
        blurImage={this.state.blurImage}
      >
        <Gradient position={"top"} size={"big"} />

        {this.state.showImagePreview && (
          <PanelImageReveal imageUrl={this.props.giftPart.photo} />
        )}
        ...
        <AccordionTitle
          ...
          textColour={"white"}
        >
          {this.props.content.partText}{" "}
          {romanNumeralFromDecimal(giftPartIndex + 1)}
        </AccordionTitle>
